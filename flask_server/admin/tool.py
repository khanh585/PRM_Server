...
def getToolByTribulationID(id):
    try:
        result = None
        result = [tool.serialize() for tool in ToolDAO.dbGetByTribulationID(id)]
        return jsonify(result), 200 
    except Exception as e:
        print(e)
        return "Server error", 500
...
def index():
    ...
    try:
        ...
        if result > 0:
            ...
            log = LogDTO(user_id = user_id, action = "create tool", date_create = datetime.now())
            LogDAO.dbCreate(log)
...
def delete(id):
    try:
        ...
        if result > 0:
            user_id = request.headers['UserID']
            log = LogDTO(user_id = user_id, action = "delete tool id: " + str(id), date_create = datetime.now())
            LogDAO.dbCreate(log)
            return jsonify(result), 200
...
def update(id):
    ...
    try:
        ...
        if result > 0:
            user_id = request.headers['UserID']
            log = LogDTO(user_id = user_id, action = "update tool tool id:" + str(id), date_create = datetime.now())
            LogDAO.dbCreate(log)
            return jsonify(result), 200
        return "Can't delete", 403
    except Exception as e:
